...
#define FUNC_TIMER_WITH_CALL_NAME(IDENTITY) \
  auto temp8234901684 = FuncTimer(          \
      IDENTITY, reinterpret_cast<uint64_t>(__builtin_return_address(0)));
#define FUNC_TIMER_WITH_RETURN_NAME(RETURN_NAME)                             \
  auto temp8234901684 = FuncTimer(                                           \
      __FUNCTION__, reinterpret_cast<uint64_t>(__builtin_return_address(0)), \
      RETURN_NAME);
...
#define OUTPUT_FUNC_EACH_PROC_TIME(OUTPUT_FILE_NAME) \
  WallTimer::GetInstance().OutputFuncEachTime(OUTPUT_FILE_NAME);
...
class WallTimer {
 private:
  class funcLogger {
   ...
   public:
    ...
    funcLogger(const std::string &func, const uint64_t &start_pc,
    ...
    void StartLogger(const std::string &func, const uint64_t &start_pc,
                     const uint64_t &return_pc) {
      funcName = func;
      startPC = start_pc;
      returnPC = return_pc;
    ...
    void EndLogger(const uint64_t &end_pc) {
      ...
      if (endTime.count(thread_id)) {
        ...
        diffTime[thread_id].push_back(
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                endTime[thread_id].back() - startTime[thread_id].back())
                .count());
      } else {
    ...
    for (auto &ft : funcTime) {
      ...
      if (!func.ReturnFuncSearched()) {
        for (auto &tft : funcTime) {
          ...
          if (tfunc.StartPC() < func.ReturnPC() &&
              func.ReturnPC() < tfunc.EndPC()) {
 ...
 public:
  ...
  void StartFuncTime(const std::string &func, const uint64_t &start_pc,
                     const uint64_t &return_pc,
                     const std::string &return_func = "") {
    // 要素がある場合は1、ない場合は0
    std::lock_guard<std::mutex> lock(mtx_);
  ...
  void EndFuncTime(const std::string &func, const uint64_t &return_pc,
                   const uint64_t &end_pc) {
    std::lock_guard<std::mutex> lock(mtx_);
    auto tuple = std::tuple<std::string, uint64_t>(func, return_pc);
  ...
  void OutputRapTime(std::string filename = "") {
    ...
    for (size_t i = 1; i < rapTime.size(); ++i) {
      // auto count =
      // std::chrono::duration_cast<std::chrono::nanoseconds>(rapTime[i].time_since_epoch()).count();
      auto count = std::chrono::duration_cast<std::chrono::nanoseconds>(
                       rapTime[i - 0] - rapTime[i - 1])
                       .count();
      ost << count << ",";
    }
    ost << std::endl;
